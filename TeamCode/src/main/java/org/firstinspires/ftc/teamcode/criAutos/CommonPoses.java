package org.firstinspires.ftc.teamcode.criAutos;

import org.firstinspires.ftc.teamcode.pedropathing.localization.Pose;

public class CommonPoses {
    public static final Pose startLeftOuter = new Pose();
    public static final Pose startLeftInner = new Pose();
    public static final Pose startRightInner = new Pose();
    public static final Pose startRightOuter = new Pose();


    public static final Pose redBasket  = new Pose();
    public static final Pose blueBasket = new Pose();

    public static final Pose lowBarLeftOuter  = new Pose();
    public static final Pose highBarLeftOuter = new Pose();

    public static final Pose lowBarLeftInner  = new Pose();
    public static final Pose highBarLeftInner = new Pose();

    public static final Pose lowBarRightInner  = new Pose();
    public static final Pose highBarRightInner = new Pose();

    public static final Pose lowBarRightOuter  = new Pose();
    public static final Pose highBarRightOuter = new Pose();

    public static final Pose placeOffsetFarther = new Pose();
    public static final Pose placeOffsetCloser  = new Pose();
}
