...
public class OTOSLocalizer extends Localizer {
    ...
    private Pose previousSmoothedVelocity = new Pose();
    private static final double ALPHA = 0.2; // tuning parameter for Exponential Moving Average
    ...
    public void update() {
        ...
        if (poseHistory.size() >= 2) {
            ...
            if (deltaTime > 0) {
                ...
                Pose rawVelocity = new Pose(dx / deltaTime, dy / deltaTime, dh / deltaTime);

                double smoothedX = ALPHA * rawVelocity.getX() + (1 - ALPHA) * previousSmoothedVelocity.getX();
                double smoothedY = ALPHA * rawVelocity.getY() + (1 - ALPHA) * previousSmoothedVelocity.getY();
                double smoothedH = ALPHA * rawVelocity.getHeading() + (1 - ALPHA) * previousSmoothedVelocity.getHeading();

                calculatedVelocity = new Pose(smoothedX, smoothedY, smoothedH);

                previousSmoothedVelocity = calculatedVelocity;

                // Optional: calculate acceleration
                double ddx = (calculatedVelocity.getX() - previousSmoothedVelocity.getX()) / deltaTime;
                double ddy = (calculatedVelocity.getY() - previousSmoothedVelocity.getY()) / deltaTime;
                double ddh = (calculatedVelocity.getHeading() - previousSmoothedVelocity.getHeading()) / deltaTime;

                calculatedAcceleration = new Pose(ddx, ddy, ddh);
            }
        }
    }
    ...
    public void resetOTOS() {
        ...
        previousSmoothedVelocity = new Pose();
        calculatedVelocity = new Pose();
        calculatedAcceleration = new Pose();
    }
